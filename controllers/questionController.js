...
async function deleteHtmlQuestion(req, res, id) {
  try {
    deletedQuestion = await Question.findHtmlById(id);
    if (!deletedQuestion) {
      res.writeHead(409, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({ message: "Question was not deleted from database" })
      );
    } else {
      await Question.deleteHtmlQuestion(id);
      res.writeHead(204, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({ message: `Question with id ${id} has been removed` })
      );
    }
  } catch (erorr) {
    console.log(error);
...
async function deleteCssQuestion(req, res, id) {
  try {
    deletedQuestion = await Question.findCssById(id);
    if (!deletedQuestion) {
      res.writeHead(409, { "Content-Type": "application/json" });
      res.end(
        JSON.stringify({ message: "Question was not deleted from database" })
      ...
      await Question.deleteCssQuestion(id);
      res.writeHead(204, { "Content-Type": "application/json" });
